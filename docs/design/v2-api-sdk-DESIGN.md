...
**Scope:** v2 serializers, URL routing, error standardization, OpenAPI spec.

**Deliverables:**
- `src/*/serializers_v2.py` — v2 serializer classes for all entities
- `src/core/versioning.py` — `VersionedSerializerMixin`
- `src/core/errors.py` — Standardized error response handler
- `src/core/idempotency.py` — Idempotency-Key middleware
- `src/vtaskforge/urls.py` — `/v2/` URL prefix routing
- OpenAPI spec generated via `drf-spectacular`
- Unit tests: every serializer output matches the contract in this document
- Integration tests: v2 endpoints return correct shapes
...
**Deliverables:**
- `@vtf/sdk` — core client, entity types, managers
- `@vtf/sdk-react` — React Query hooks, SSE integration, mutation hooks
