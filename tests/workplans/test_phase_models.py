...
class TestPhaseModel:
    def test_create_phase_minimal(self, workplan):
        phase = PhaseFactory(name="Phase 1", workplan=workplan)
    ...
    def test_review_flags_default_to_null(self, workplan):
        phase = PhaseFactory(name="Phase", workplan=workplan)
    ...
    def test_str_representation(self, workplan):
        phase = PhaseFactory(name="My Phase", workplan=workplan)
        assert str(phase) == "My Phase"

    def test_nanoid_primary_key(self, workplan):
        p1 = PhaseFactory(name="Phase 1", workplan=workplan)
        p2 = PhaseFactory(name="Phase 2", workplan=workplan)
        assert p1.id != p2.id
        assert len(p1.id) == 21
    ...
    def test_workplan_related_name(self, workplan):
        ...
        PhaseFactory(name="Phase B", workplan=workplan)
        assert workplan.phases.count() == 2

    def test_default_ordering(self, workplan):
        p1 = PhaseFactory(name="Phase 1", workplan=workplan, order=2)
        p2 = PhaseFactory(name="Phase 2", workplan=workplan, order=1)
