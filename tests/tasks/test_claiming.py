...
# ---------------------------------------------------------------------------
# Auto-membership grant on claim
# (closes vtaskforge#2; see docs/agent-project-membership-DECISION.md)
# ---------------------------------------------------------------------------


@pytest.mark.django_db
class TestClaimAutoMembership:
    """Claiming a task auto-grants the agent's user a ProjectMembership for
    the task's project, so subsequent membership-filtered list/poll calls
    return non-empty results. Idempotent and audited.
    """

    def test_first_claim_creates_project_membership(
        self, api_client, milestone, workplan, agent1
    ):
        from prefs.models import ProjectMembership
        task = make_task(milestone, workplan)
        assert not ProjectMembership.objects.filter(
            user=agent1.user, project_id=task.project_id
        ).exists()
    ...
    def test_first_claim_records_audit_event(
        self, api_client, milestone, workplan, agent1
    ):
        ...
        assert event.data["project_id"] == task.project_id
        assert event.data["user_id"] == agent1.user.id
        assert event.data["agent_id"] == agent1.id
        assert event.data["source"] == "claim"

    def test_repeat_claim_is_idempotent_no_duplicate_membership(
        self, api_client, milestone, workplan, agent1
    ):
        """Claiming a second task in the same project must not duplicate
        membership (unique constraint would prevent it anyway, but the
        get_or_create path also must not raise).
        """
        from prefs.models import ProjectMembership
        task1 = make_task(milestone, workplan, title="t1")
        task2 = make_task(milestone, workplan, title="t2")
        api_client.post(
            f"/v1/tasks/{task1.id}/claim/", {"agent_id": agent1.id}, format="json"
        )
        # Free up the agent so the second claim isn't blocked
        task1.refresh_from_db()
        api_client.post(
            f"/v1/tasks/{task2.id}/claim/", {"agent_id": agent1.id}, format="json"
        )
        memberships = ProjectMembership.objects.filter(
            user=agent1.user, project_id=task1.project_id
        )
        assert memberships.count() == 1

    def test_repeat_claim_does_not_re_emit_audit_event(
        self, api_client, milestone, workplan, agent1
    ):
        from events.models import TaskEvent
        task1 = make_task(milestone, workplan, title="t1")
        ...
        # Only the first claim should have emitted the grant event
        events = TaskEvent.objects.filter(event_type="auto_membership_grant")
        assert events.count() == 1
        # And it should be on task1, not task2
        assert events.first().task_id == task1.id

    def test_pre_existing_membership_does_not_re_emit_audit_event(
        self, api_client, milestone, workplan, agent1
    ):
        """If the agent's user is already a member (operator added them
        manually, or previous claim), no audit event should fire on the
        next claim.
        """
        from prefs.models import ProjectMembership
        from events.models import TaskEvent
        ...
        ProjectMembership.objects.create(
            user=agent1.user, project_id=task.project_id, role="member"
        )
        api_client.post(
            f"/v1/tasks/{task.id}/claim/", {"agent_id": agent1.id}, format="json"
        )
    ...
    def test_claim_failure_does_not_create_membership(
        self, api_client, milestone, workplan, agent1
    ):
        """A claim that fails (e.g. tag mismatch) must roll back the
